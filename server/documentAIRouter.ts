        ...
        .mutation(async ({ input, ctx }) => {
            ...
            if (input.trialId && input.trialId !== "all") {
                ...
                if (UUID_RE.test(input.trialId)) {
                    // Raw UUID — exists only in core-backend, no MySQL prefix needed
                    resolvedTrialId = input.trialId;
                } else {
                    resolvedTrialId = await resolveTrialId(db, mode, input.trialId, mode !== "building");
                }
            }
...
${conversationHistory}`;
            ...
            // Detect if documentIds are UUIDs (core-backend IDs) vs numeric (local MySQL IDs)
            const UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
            const uuidDocIds = input.documentIds.filter(id => UUID_PATTERN.test(id));

            if (uuidDocIds.length > 0 && ENV.coreBackendApiUrl) {
                // Fetch actual doc metadata from FastAPI to get fileUrl for source cards
                let trialDocs: any[] = [];
                if (resolvedTrialId) {
                    try {
                        trialDocs = await callBackend<any[]>(
                            `/api/trial-documents/?trial_id=${resolvedTrialId}`,
                            { method: "GET" }
                        ) ?? [];
                    } catch {
                        // ignore — fall back to no fileUrl
                    }
                }

                const enrichedDocs = uuidDocIds.map(docId => {
                    const match = trialDocs.find((d: any) => d.id === docId);
                    return {
                        id: 0,
                        coreBackendDocumentId: docId,
                        filename: match?.document_name || docId,
                        fileUrl: match?.document_url || null,
                        category: match?.document_type || null,
                        trialId: resolvedTrialId ?? "",
                    } as IndexedProtocolRow;
                });
                console.log("[Document AI] enrichedDocs:", JSON.stringify(enrichedDocs.map(d => ({
                    id: d.coreBackendDocumentId,
                    filename: d.filename,
                    fileUrl: d.fileUrl,
                }))));

                const beResult = await queryViaCoreBackend({
                    coreBackendDocs: enrichedDocs,
                    messages: input.messages,
                    resolvedTrialId,
                    sessionId: input.sessionId,
                    user: ctx.user,
